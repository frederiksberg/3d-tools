...
def read_shape(path):
    shp_read = shapefile.Reader(path)
    return shp_read
...
def polygon_caculation(inits, points_2D):
    ...
    dachhoehe = inits['elev_mean'] + grundhoehe
...
def building_inits(content, shp_layer):
    ...
    for i, field in zip(range(len(shp_layer.fields) - 1), shp_layer.fields[1:]):
        ...
        if field[0] == 'gid':
            inits['gid'] = content[i]
        elif field[0] == 'bygninguui':
            inits['bygninguui'] = content[i]
        elif field[0] == 'gml_id':
            inits['gml_id'] = content[i]
        elif field[0] == 'elev_mean':
            inits['elev_mean'] = content[i]
        elif field[0] == 'elev_min':
            inits['elev_min'] = content[i]
        elif field[0] == 'elev_max':
            inits['elev_max'] = content[i]
...
def iteration_buildings(cityModel, shp_layer, ns_core, ns_bldg, ns_gen, ns_gml, ns_xAL, ns_xlink, ns_xsi):
    ...
    for i_build in range(building_count):
        ...
        point_min, point_max = find_lower_upper_corner(points_2D, inits['elev_mean'], point_min, point_max)
        ...
        bldg = etree.SubElement(cityObject, "{%s}Building" % ns_bldg, {"{%s}id" % ns_gml: str(inits['gid'])})
        ...
        name.text = str(inits['gid'])

        stringAttribute = etree.SubElement(bldg, "{%s}stringAttribute" % ns_gen, name="bygninguui")
        value = etree.SubElement(stringAttribute, "{%s}value" % ns_gen)
        value.text = inits['bygninguui']
        ...
        measuredHeight.text = str(inits['elev_mean'])
        ...
        for poly in polygon:
            ...
            polygon = etree.SubElement(surfaceMember, "{%s}Polygon" % ns_gml, {"{%s}id" % ns_gml: str(inits['gid'])})
        ...
        print('progress:', i_build, building_count)
...
def build_gml_main():    
    ...
    ns_gen = "hhttp://www.opengis.net/citygml/generics/2.0"
    ...
    envelop = etree.SubElement(bounded, "{%s}Envelope" % ns_gml, srsName="urn:ogc:def:crs:EPSG::25832")
    ...
    # # Read Shapefile
    shp_path = './data/building_frb.shp'
    shp_layer = read_shape(shp_path)
    ...
    outFile = open('./data/building_frb.gml', 'wb')
...
if __name__ == "__main__":
    ...
    # ET.register_namespace('', "http://www.opengis.net/citygml/building/2.0")


    # tree = ET.parse("./data/template.gml")
    # root = tree.getroot()
    # #a = root.find('CityModel')          # Get parent node from EXISTING tree
    # b = ET.SubElement(root,"cityObjectMember")
    # b.text = "Jay/Doctor"
    ...
    # path = './data/building.shp'
    # shp = read_shape(path)
    # attribute(shp)
    #print(shp.shapes()[0].points)
